    ...
    if (typeof item.body.setSignatureAsync === "function") {
        ...
        if (sigBytes <= SET_SIGNATURE_ASYNC_MAX_BYTES) {
            try {
                ...
                _diag.warn("PATH A step_setSig threw: " + (e?.message || JSON.stringify(e))
                    + " — falling through to PATH A→B");
            }
        } else {
        ...
        _diag.info("PATH A→B step_write: " + byteKB(combined).toFixed(1) + " KB");
        try {
            await _setBody(item, combined);
            _diag.info("PATH A→B step_write: OK");
        } catch (e) {
            _diag.error("PATH A→B step_write failed: " + (e?.message || JSON.stringify(e)));
            return false;
        }
        await _moveCursorToTop(item);
        try {
            ...
            _diag.info("PATH A→B step_verify: CB_SIG_START=" + startOk + " | CB_SIG_END=" + endOk);
            _diag.info("=== writeSignature END | success=" + (startOk && endOk) + " (PATH A→B) ===");
            return startOk && endOk;
        } catch (e) {
            _diag.warn("PATH A→B step_verify: read failed — assuming success");
            return true;
        }
    }

...
import { Stage, Layer, Text, Image as KonvaImage, Transformer, Line, Rect, Circle, Group } from "react-konva";
import { Box, Button, CircularProgress, Dialog, Divider, IconButton, Input, Paper, Stack, Typography } from "@mui/material";
import useImage from "use-image";
import { toast } from "react-toastify";
import { QRCode } from "react-qrcode-logo";
import { createRoot } from "react-dom/client";
import { generateEmailSignatureHTML, IconAvatar } from "./IconAvatar";
import { KeyboardArrowDownOutlined } from "@mui/icons-material";
import GlobeSVG from "./Assets/SvgComponents/GlobeSVG";
import FaxxSVG from "./Assets/SvgComponents/FaxxSVG";
import EmailSVG from "./Assets/SvgComponents/EmailSVG";
import PhoneSVG from "./Assets/SvgComponents/PhoneSVG";
import MobileSVG from "./Assets/SvgComponents/MobileSVG";
...
export default function PreviewEmailSignatureInAlert({ apply = () => { }, showPreview = false, setShowPreview = () => { }, showSocialMediaIcons = true, showScheduler = true, form = {}, showStack = false, Card = null, shortLink = "" }) {
    const containerRef = useRef(null);
    const stageRef = useRef(null);
    const card = Card;
    const qrcodeLogos = [];

    const DefaultQrCodeLogo = qrcodeLogos?.find(qrcodeLogos => qrcodeLogos?.isDefault === true) || {};
    const baseWidth = 336;
    ...
    const updateFieldsFromCard = (card) => (prevFields) => {
        ...
        return prevFields.map((field) => {
            ...
            if (field.key === "profilePhoto") {
                return {
                    ...field,
                    value: "https://newqa-enterprise.cardbyte.ai/v2/getCardProfileImage/5badb545-d26c-4c83-aee2-a49070be7792_1764839588156_profileImage.png",
                    show: field.show,
                };
            }
    ...
    const generateHtmlFromStage = async () => {
        try {
            ...
            console.log("Generating HTML from stage...", html);
            ...
            apply(html)
            alert("Signature HTML copied to clipboard! Now paste directly into Gmail/Outlook.");
        } catch (e) {
            console.error(e)
        }
    }
    ...
    return (
        ...
        >
                        ...
                        <Layer>
                            ...
                            {backgroundImage && (
                                <KonvaImage
                                    ...
                                    x={0}
                                    y={0}                      // ✅ Background starts at the top
                                    width={stageSize.width}
                                    height={stageSize.height}
                                    cornerRadius={[8, 8, 8, 8]} // only bottom corners rounded
                                    listening={false}
                            ...
                            {!form?.elements?.find(f => f.key === "disclaimer")?.value && (
                                <Text
                                    ...
                                    // text={form?.find(f => f.key === "disclaimer").value}
            ...
            <Stack spacing={1} direction="row" justifyContent={'end'} sx={{ mt: 2 }}>
                <Button
                    onClick={generateHtmlFromStage}
                    variant="outlined"
                    size="small"
                    sx={{
                        width: "100px",

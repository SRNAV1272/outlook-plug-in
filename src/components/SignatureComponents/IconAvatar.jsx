...
export function generateEmailSignatureHTML(dataURL, allFields = {}) {
    if (!dataURL) return "";

    // -------- 1️⃣ Extract useful fields -------- //
    const disclaimerField = allFields.find(f => f.key === "disclaimer" && f.show);
    const bannerField = allFields.find(f => f.key === "banner" && f.show);
    ...
    // -------- 3️⃣ Optional Banner -------- //
    const bannerHTML = bannerField.link
        ? `
            <tr>
